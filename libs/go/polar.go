...
func renderPolarSVG(spec *ChartSpec) string {
	...
	a11yDesc := ""
	if spec.a11yOn() {
		sum := esc(a11ySummary(spec, "Polar"))
	...
	for level := 0; level < nRings; level++ {
		...
		r := rMax * frac
		p.WriteString(fmt.Sprintf(
			`<circle class="sc-polar-ring" data-level="%d" cx="%s" cy="%s" r="%s" fill="none" stroke="%s" stroke-width="1"/>`,
	...
	for i := 0; i < nAxes; i++ {
		...
		p.WriteString(fmt.Sprintf(
			`<line class="sc-polar-axis" data-index="%d" x1="%s" y1="%s" x2="%s" y2="%s" stroke="%s" stroke-width="1"/>`,
	...
	for i := 0; i < nAxes; i++ {
		...
		p.WriteString(fmt.Sprintf(
			`<text class="sc-polar-label" data-index="%d" x="%s" y="%s" text-anchor="%s" dominant-baseline="central" font-size="11" fill="%s">%s</text>`,
	...
	for level := 0; level < nRings; level++ {
		...
		p.WriteString(fmt.Sprintf(
			`<text class="sc-polar-tick" data-value="%s" x="%s" y="%s" font-size="9" fill="%s">%s</text>`,
	...
	for si, s := range spec.Series {
		...
		p.WriteString(fmt.Sprintf(
			`<path class="sc-polar-poly sc-point" data-series="%d" data-series-name="%s" data-color="%s" d="%s" %s%s stroke="%s" stroke-width="2"/>`,
		...
		for j, vt := range vertices {
			...
			p.WriteString(fmt.Sprintf(
				`<circle class="sc-polar-dot sc-point" data-series="%d" data-index="%d" data-y="%s" cx="%s" cy="%s" r="4" fill="%s"/>`,
