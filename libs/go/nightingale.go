...
func renderNightingaleSVG(spec *ChartSpec) string {
	...
	a11yDesc := ""
	if spec.a11yOn() {
		sum := esc(a11ySummary(spec, "Nightingale"))
	...
	for level := 0; level < nRings; level++ {
		...
		r := rMax * frac
		p.WriteString(fmt.Sprintf(
			`<circle class="sc-nightingale-ring" data-level="%d" cx="%s" cy="%s" r="%s" fill="none" stroke="%s" stroke-width="1"/>`,
	...
	for i := 0; i < nDirs; i++ {
		...
		p.WriteString(fmt.Sprintf(
			`<line class="sc-nightingale-axis" data-index="%d" x1="%s" y1="%s" x2="%s" y2="%s" stroke="%s" stroke-width="1"/>`,
	...
	for i := 0; i < nDirs; i++ {
		...
		p.WriteString(fmt.Sprintf(
			`<text class="sc-nightingale-label" data-index="%d" x="%s" y="%s" text-anchor="%s" dominant-baseline="central" font-size="11" fill="%s">%s</text>`,
	...
	for level := 0; level < nRings; level++ {
		...
		p.WriteString(fmt.Sprintf(
			`<text class="sc-nightingale-tick" data-value="%s" x="%s" y="%s" font-size="9" fill="%s">%s</text>`,
			esc(fmtNum(tickVal)), f1(tx+4), f1(ty2-2), theme.AxisLabelColor, esc(fmtNum(tickVal))))
	}

	// Sector wedges (from center, overlaid per series)
	...
	for si, s := range spec.Series {
		...
		for j := 0; j < nDirs; j++ {
			...
			ox1, oy1 := pointAt(aStart, r)
			ox2, oy2 := pointAt(aEnd, r)
			...
			pathD := fmt.Sprintf("M %s %s L %s %s A %s %s 0 0 1 %s %s Z",
				f1(cx), f1(cy),
				f1(ox1), f1(oy1),
				f1(r), f1(r), f1(ox2), f1(oy2))
			...
			p.WriteString(fmt.Sprintf(
				`<path class="sc-nightingale-sector sc-point" data-series="%d" data-index="%d" data-y="%s" data-color="%s" d="%s" %s%s stroke="%s" stroke-width="1"/>`,
				si, j, esc(fmtNum(v)), color, pathD, fillAttr, opacityAttr, strokeColor))
		}
	}
