"""Nightingale / rose / coxcomb chart renderer: ChartSpec -> SVG string.

Polar column where radius encodes value at equal angles — sector wedges from
center. Multiple series overlaid (back-to-front) with fill opacity.
Non-Cartesian (Family B polar variant) — own SVG shell, no axes.
See charts/nightingale/design.md for the full geometry contract.
"""
...
def render_svg(spec: ChartSpec) -> str:
    ...
    a11y_desc = ""
    if spec.a11y:
        _sum = esc(a11y_summary(spec, "Nightingale"))
    ...
    for level in range(n_rings):
        ...
        r = r_max * frac
        p.append(
            f'<circle class="sc-nightingale-ring" data-level="{level}" '
    ...
    for i in range(n_dirs):
        ...
        p.append(
            f'<line class="sc-nightingale-axis" data-index="{i}" '
    ...
    for i in range(n_dirs):
        ...
        p.append(
            f'<text class="sc-nightingale-label" data-index="{i}" '
    ...
    for level in range(n_rings):
        ...
        p.append(
            f'<text class="sc-nightingale-tick" data-value="{esc(fmt_num(tick_val))}" '
    ...
    for si, s in enumerate(spec.series):
        ...
        for j in range(n_dirs):
            ...
            ox1, oy1 = point_at(a_start, r)
            ox2, oy2 = point_at(a_end, r)

            path_d = f"M {cx:.1f} {cy:.1f} L {ox1:.1f} {oy1:.1f} A {r:.1f} {r:.1f} 0 0 1 {ox2:.1f} {oy2:.1f} Z"

            fill_attr = f'fill="{color}"' if fill_opacity > 0 else 'fill="none"'

            p.append(
                f'<path class="sc-nightingale-sector sc-point" data-series="{si}" '
                f'data-index="{j}" data-y="{esc(fmt_num(v))}" data-color="{color}" '
                f'd="{path_d}" {fill_attr}{opacity_attr} '
                f'stroke="{theme.background or "#fff"}" stroke-width="1"/>'
            )
