"""Polar chart renderer: ChartSpec -> SVG string.

Categorical data on a circular polar grid with circular gridlines.
Non-Cartesian (Family B polar sibling) — own SVG shell, no axes.
See charts/polar/design.md for the full geometry contract.
"""
...
def render_svg(spec: ChartSpec) -> str:
    ...
    a11y_desc = ""
    if spec.a11y:
        _sum = esc(a11y_summary(spec, "Polar"))
    ...
    for level in range(n_rings):
        ...
        r = r_max * frac
        p.append(
            f'<circle class="sc-polar-ring" data-level="{level}" '
    ...
    for i in range(n_axes):
        ...
        p.append(
            f'<line class="sc-polar-axis" data-index="{i}" '
    ...
    for i in range(n_axes):
        ...
        p.append(
            f'<text class="sc-polar-label" data-index="{i}" '
    ...
    for level in range(n_rings):
        ...
        p.append(
            f'<text class="sc-polar-tick" data-value="{esc(fmt_num(tick_val))}" '
    ...
    for si, s in enumerate(spec.series):
        ...
        p.append(
            f'<path class="sc-polar-poly sc-point" data-series="{si}" '
        ...
        for j, (vx, vy) in enumerate(vertices):
            v = s.data[j] if j < len(s.data) else 0.0
            p.append(
                f'<circle class="sc-polar-dot sc-point" data-series="{si}" '
