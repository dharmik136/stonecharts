...
Shared Cartesian chrome comes from _cartesian.py. This module draws only bar
marks: one <rect> per category/series segment, with the value axis transposed
onto x and the category bands running down y.
"""
    ...
    for si, s in enumerate(fr.spec.series):
        ...
        for i, raw in enumerate(s.data):
            ...
            if stacked:
                ...
                slot = si if fr.spec.grouping else 0
                top = cy_band - group_h / 2 + bar_h * slot
                xv = fr.value_pix(raw)
                x = min(baseline, xv)
                w = abs(baseline - xv)
                cx = xv
