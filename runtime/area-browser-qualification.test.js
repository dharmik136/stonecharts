...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "stonecharts-area-browser-"));
}
...
spec = ChartSpec(
    type="area",
    title="Area Browser Qualification",
...
test("area chart local HTTP Chromium browser qualification covers runtime and a11y contracts", async () => {
  const tmpDir = makeTempDir();
  ...
  try {
    ...
    // Keyboard navigation bypasses the area fill overlay issue entirely.
    // Focus activates (0,0); first ArrowRight advances to (0,1).
    await svg.focus();
    ...
    assert.match(tipText, /Alpha/);
    assert.match(tipText, /Feb/);

    // ArrowDown from (0,1) moves to (1,1) — Beta, Feb.
    await page.keyboard.press("ArrowDown");
    assert.equal(await point01.getAttribute("r"), "3.5");
    assert.equal(await point11.getAttribute("r"), "6");

    // Escape clears the highlight but keeps focus on the SVG.
    ...
    // Legend toggle hides/shows a series.
    await legend0.click();
