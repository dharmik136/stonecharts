// Browser qualification for bar specifically. bar.py/bar.go emit the exact
// same DOM contract as line/column (.sc-point, .sc-series, data-*) per
// charts/bar/design.md - "orientation changes none of them, the runtime is
// unchanged" - but that claim was never directly verified in a live browser
// before this file existed (chart-admission-checklist.md Phase 6). This
// mirrors browser-qualification.test.js's exact assertions against a bar
// chart instead of line, rather than inferring bar works because line's test
// passes.
...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "stonecharts-bar-browser-"));
}
...
spec = ChartSpec(
    type="bar",
    title="Bar Browser Qualification",
...
test("bar chart local HTTP Chromium browser qualification covers runtime and a11y contracts", async () => {
  const tmpDir = makeTempDir();
