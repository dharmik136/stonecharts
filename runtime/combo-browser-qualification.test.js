...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "stonecharts-combo-browser-"));
}
...
spec = ChartSpec(
    type="combo",
    title="Combo Browser Qualification",
    x_axis=Axis(categories=["Jan", "Feb"]),
    series=[
        Series(name="Revenue", data=[10, 20], type="column"),
        Series(name="Trend", data=[12, 18], type="line"),
    ],
)
...
test("combo chart local HTTP Chromium browser qualification covers runtime and a11y contracts", async () => {
  const tmpDir = makeTempDir();
  ...
  try {
    ...
    assert.match(hoverText, /Revenue/);
